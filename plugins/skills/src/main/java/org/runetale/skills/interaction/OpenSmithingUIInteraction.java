...
/**
 * Block interaction that opens the custom smithing UI page.
 *
 * <p>
 * Registered with {@link com.hypixel.hytale.server.core.modules.interaction.interaction.config.Interaction#CODEC}
 * as {@code "runetale_open_smithing"}. Follows the {@link SimpleInstantInteraction} pattern
 * to avoid the codec loading-order issue with {@code OpenCustomUIInteraction.PAGE_CODEC}.
 */
public final class OpenSmithingUIInteraction extends SimpleInstantInteraction {

	public static final String TYPE_NAME = "runetale_open_smithing";

	public static final BuilderCodec<OpenSmithingUIInteraction> CODEC = BuilderCodec
			.builder(OpenSmithingUIInteraction.class, OpenSmithingUIInteraction::new, SimpleInstantInteraction.CODEC)
			.build();
	...
	protected void firstRun(
		...
		if (targetBlock == null) {
			LOGGER.atWarning().log("No target block for smithing interaction");
			ctx.getState().state = InteractionState.Failed;
			return;
		}
